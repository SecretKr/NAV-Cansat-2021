...
void loop() {
  ...
  if((launch < 2 && vec >= 2) || t>=30) launch=2;
...
//palord 
  if(!digitalRead(3) && launch==2) repal++;
  else repal = 0;
  
  
  String mm = String(millis())+","+String(t)+","+String(aX)+","+String(aY)+","+String(aZ)+","+String(vec)+","+String(rempu)+","+String(repal);
  if(bmpflag) mm += ","+String(bmp.readTemperature())+","+String(bmp.readPressure()/100)+","+String(bmp.readAltitude(1019.66))+","+String(gX)+","+String(gY)+","+String(gZ);
  myFile = SD.open("log.csv", FILE_WRITE);
  if(myFile){
    myFile.println(mm);
    myFile.close();
