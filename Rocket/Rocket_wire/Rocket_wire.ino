...
void setup()
{
  ...
  if(myFile){
    myFile.println("\nMillis,T,AX,AY,AZ,Vec,ReMPU,wire,Temp,Press,Alt");
    myFile.close();
  }
  while(digitalRead(3));
}
...
void loop() {
  ...
  if(launch < 2 && digitalRead(3)) launch++;
...
//mpu
  ...
  String mm = String(start)+","+String(t)+","+String(aX)+","+String(aY)+","+String(aZ)+","+String(vec)+","+String(rempu)+","+String(digitalRead(3));
