...
//mpu
  if(vec <= zg && launch==2) releaseMPU++;
  else releaseMPU = 0;

//palord 
  if(!digitalRead(3) && launch==2 && t>=80) releasePalord++;
  else releasePalord = 0;

  if(t==80 && releaseMPU>3){
    releaseMPU-=20;
  }
  
  
  String mm = String(millis())+","+String(t)+","+String(aX)+","+String(aY)+","+String(aZ)+","+String(vec)+","+String(releaseMPU)+","+String(releasePalord);
  ...
  if((releaseMPU >= 3  && t>=80) || releasePalord >= 4 || t>140){
    servo.write(o);
  }
  //else servo.write(c);
